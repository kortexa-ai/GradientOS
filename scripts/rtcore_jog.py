...
def _print_status(s: StatusSnapshot) -> None:
    ...
    for i in range(s.num_axes):
        ...
        print(
            ...
            f"err=0x{a.error_code:04x} mode_disp={a.mode_display} "
...
def main() -> int:
    ...
    g.add_argument("--delta-rad", type=float, help="Relative delta in radians (rotary)")
    ...
    try:
        with RTCoreClient(args.socket) as c:
            ...
            if args.cmd == "console":
                ...
                cpr = _counts_per_rad(int(args.counts_per_rev), float(args.gear_ratio))
                if cpr <= 0:
                ...
                safe_print_block(
                    [
                        ...
                        "  set AXIS Q | jog AXIS DELTA_RAD | jogc AXIS DELTA_COUNTS",
                        "  quit",
                    ]
                )
                ...
                try:
                    while True:
                        ...
                        if cmd0 == "help":
                            safe_print_block(
                                [
                                    ...
                                    "          set AXIS Q, jog AXIS DELTA_RAD, jogc AXIS DELTA_COUNTS, quit",
                                ]
                            )
                            continue
                        if cmd0 in {"w", "watch"}:
                        ...
                        if cmd0 == "status":
                            ...
                            for i in range(snap.num_axes):
                                ...
                                lines.append(
                                    ...
                                    f"err=0x{a.error_code:04x} mode_disp={a.mode_display} torque_raw={a.torque_raw} "
                        ...
                        try:
                            ...
                            if cmd0 in {"jog", "jogc"}:
                                if len(parts) < 3:
                                    safe_print_line("Usage: jog AXIS DELTA_RAD  |  jogc AXIS DELTA_COUNTS")
                                    continue
                                axis = int(parts[1])
                                snap = last_snap
                                if snap is None:
                                ...
                                if cmd0 == "jogc":
                                    ...
                                    target_q = float(target_counts) / (float(args.sign) * cpr)
                                else:
                                    delta_rad = float(parts[2])
                                    target_q = (float(cur_counts) / (float(args.sign) * cpr)) + delta_rad
                                q = [0.0] * _MAX_AXES
                                q[axis] = float(target_q)
                                with io_lock:
                                    c.write_setpoint(q, axis_mask=(1 << axis))
                                safe_print_line(f"jog axis{axis}: pos_counts={cur_counts} -> q_target={target_q:.6f}")
                                continue
            ...
            if args.cmd == "jog":
                ...
                if args.delta_counts is not None:
                    ...
                    target_q = (float(cur_counts) / (float(args.sign) * cpr)) + float(args.delta_rad)

                q = [0.0] * _MAX_AXES
                q[axis] = float(target_q)
                c.write_setpoint(q, axis_mask=(1 << axis))
                print(f"Jog axis{axis}: pos_counts={cur_counts} -> q_target={target_q:.6f}")
                return 0
